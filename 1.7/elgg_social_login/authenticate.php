...
<?php
	...
	if( isset( $_GET["provider"] ) && isset( $_GET["redirect_to_provider"] )){
		try{ 
			...
			$values = array( 'plugin:settings:elgg_social_login:uid' => $user_uid );
			...
			if ( ! $users = get_entities_from_private_setting_multi( $values, 'user', '', 0, '', 0 ) ) { 
				...
				if( ! $useremail ){
					$useremail = md5( $provider . "_" . $user_profile->identifier ) . "@example.com";
				}

				// $userlogin = $userlogin . rand( );
				$user_data = array( $userlogin, $password, $username, $useremail );

				try {
					// create new account
					if ( ! $user_id = register_user( $userlogin , $password, $username, $useremail ) ) {
				...
				// register user 
				set_plugin_usersetting( 'uid', $user_uid, $user->getGUID(), 'elgg_social_login' );

				// store hybridauth session
				set_plugin_usersetting( 'hybridauth_session', Hybrid_Auth::getSessionData(), $user->getGUID(), 'elgg_social_login' );
			...
			# {{{ update user profile
				...
				// 1. About me
				create_metadata( $user->getGUID(), "description", html_entity_decode( $user_profile->description, ENT_COMPAT, 'UTF-8'), "text", $user->getGUID(), 1 );

				// 2. Brief description
				create_metadata( $user->getGUID(), "briefdescription", html_entity_decode( $user_profile->description, ENT_COMPAT, 'UTF-8'), "text", $user->getGUID(), 1 );

				// 3. Location
				create_metadata( $user->getGUID(), "location", html_entity_decode( $user_profile->region, ENT_COMPAT, 'UTF-8'), "text", $user->getGUID(), 1 );

				// 4. contactemail
				create_metadata( $user->getGUID(), "contactemail", html_entity_decode( $user_profile->email, ENT_COMPAT, 'UTF-8'), "text", $user->getGUID(), 1 );

				// 5. website
				create_metadata( $user->getGUID(), "website", html_entity_decode( $user_profile->profileURL, ENT_COMPAT, 'UTF-8'), "text", $user->getGUID(), 1 );
			# }}} update user profile
			...
			elseif (count($users) == 1) { 
				...
				// store hybridauth session
				set_plugin_usersetting( 'hybridauth_session', Hybrid_Auth::getSessionData(), $users[0]->getGUID(), 'elgg_social_login' );
  ...
  <tr>
    <td align="center"> 
		<div style="padding: 5px;margin: 5px;background: none repeat scroll 0 0 #F5F5F5;border-radius:3px;">
			<br /> 
			&nbsp;<b>This plugin is still in alpha</b><br /><br /><b style="color:#cc0000;">But you can make it better by sending this error to the developper!</b>
			<br />
			<br />

			<div id="bug_report">
				<form method="post" action="http://hybridauth.sourceforge.net/reports/index.php?product=elgg-plugin-1.7&v=1.0.2">
